...
function scenarioCard(document: WayfinderDocument, scenario: DerivedScenario) {
  ...
  return `<article class="scenario" style="--accent:${color}">
    ...
    <section class="metadata"><h3>Currency and evidence</h3><div><b>Conversion ratio</b><span>1 ${escapeHtml(displayCurrency(scenario.currency))} = ${escapeHtml(String(fxRate))} ${escapeHtml(displayCurrency(document.baseCurrency))}</span></div>${scenario.currency === document.baseCurrency ? `<div><b>Conversion</b><span>No conversion needed; this option uses the comparison currency.${scenario.fx.source ? ` ${escapeHtml(scenario.fx.source)}.` : ""}${scenario.fx.asOf ? ` Reference date: ${escapeHtml(scenario.fx.asOf)}.` : ""}</span></div>` : `<div><b>Conversion date</b><span>${escapeHtml(scenario.fx.asOf || "Date not recorded")}</span></div><div><b>Conversion source</b><span>${escapeHtml(scenario.fx.source || "No source recorded")}</span></div>`}</section>
    <section class="details"><h3>Monthly line-item breakdown</h3><p>Amounts are shown in the option currency, with the comparison-currency equivalent underneath. <span class="shared">Shared</span> items use the same comparison-currency amount in every option.</p>
...
<body>
  <main>
    <header><div><span class="eyebrow">Wayfinder · read-only family view</span><h1>${escapeHtml(document.title || "Relocation options")}</h1><p>A consistent comparison using ${escapeHtml(displayCurrency(document.baseCurrency))} as the comparison currency.</p></div><div class="stamp">Created ${escapeHtml(dateLabel)}<br>${derived.length} saved ${derived.length === 1 ? "option" : "options"}</div></header>
